...
class Web3Manager {
  ...
  async connectWallet() {
    ...
    try {
      ...
      // Проверяем правильность сети (BSC = 56)
      if (this.networkId !== 56) {
        console.warn('⚠️ Подключена неправильная сеть:', this.getNetworkName(this.networkId));
        
        if (window.globalWayApp) {
          window.globalWayApp.showNotification('Требуется переключение на BSC сеть', 'warning');
        }
        ...
        <div class="wallet-modal-overlay">
          <div class="wallet-modal-content">
            ...
            <div class="wallet-modal-body">
              ...
              <p>Для корректной работы требуется сеть Binance Smart Chain</p>
  ...
  async switchToBSC() {
    console.log('🔄 Переключение на BSC сеть...');
    ...
    try {
      ...
      this.networkId = 56;
      console.log('✅ Переключено на BSC');
      
      if (window.globalWayApp) {
        window.globalWayApp.showNotification('Переключено на Binance Smart Chain', 'success');
      }
      ...
      if (switchError.code === 4902) {
        try {
          ...
          this.networkId = 56;
          console.log('✅ BSC сеть добавлена и активирована');
          
          if (window.globalWayApp) {
            window.globalWayApp.showNotification('BSC сеть добавлена и активирована', 'success');
          }
          
        } catch (addError) {
          console.error('❌ Ошибка добавления BSC сети:', addError);
          
          if (window.globalWayApp) {
            window.globalWayApp.showNotification('Не удалось добавить BSC сеть', 'error');
          }
     ...
     if (this.networkId !== 56) {
       ...
       if (window.globalWayApp) {
         window.globalWayApp.showNotification(
           `Подключена сеть ${this.getNetworkName(this.networkId)}. Рекомендуется BSC.`, 
           'warning'
         );
       }
     } else {
       if (window.globalWayApp) {
         window.globalWayApp.showNotification('Подключена правильная сеть BSC', 'success');
       }
     }
 ...
 async signMessage(message) {
   ...
   try {
     console.log('📝 Подписание сообщения...');
 ...
 async signTypedData(typedData) {
   ...
   try {
     console.log('📝 Подписание структурированных данных...');
